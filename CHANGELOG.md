...
- **Three snap levels.** Press `Tab` while the snapper key is held to cycle Point, Edge and Face.
  ...
  merely nearby.
- **Face mode works like a door hinge, and finds its own angle.** Pick an edge on your selection and
  an edge on the target. Those two become the hinge, and the angle the selection swings to is worked
  out from the surfaces that meet at each edge - the ones that would drive the block into the target
  are discarded, and of what is left the smallest turn wins. Two picks, and the block arrives fully
  aligned instead of needing a second snap to straighten it.
